...
const BOT_NAME = 'Arusha Catholic Seminary';
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: `Hello! I can answer questions about ${BOT_NAME}—fees, admissions, student life, and NECTA results. I use only the school’s published information and don’t have access to individual student data. How can I help?` },
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const reply = res.data?.reply ?? "I couldn't get a response. Please try again or contact the school.";
      setMessages((prev) => [...prev, { role: 'bot', text: reply }]);
    } catch (err) {
      const data = err.response?.data;
      const fallback = (data && (data.reply ?? data.error)) || 'The assistant is not available right now. Please contact the school directly for questions.';
          ...
          <div className="chatbot-messages">
            {messages.map((m, i) => (
              <div key={i} className={`chatbot-msg chatbot-msg-${m.role}`}>
                {m.role === 'bot' && <i className="fas fa-robot chatbot-msg-icon"></i>}
            ...
            {loading && (
              <div className="chatbot-msg chatbot-msg-bot">
                <i className="fas fa-robot chatbot-msg-icon"></i>
            ...
            {messages.length <= 1 && !loading && (
              <div className="chatbot-suggestions">
                <span className="chatbot-suggestions-label">Suggested:</span>
                {SUGGESTIONS.map((s, i) => (
                  <button
                    key={i}
                    type="button"
