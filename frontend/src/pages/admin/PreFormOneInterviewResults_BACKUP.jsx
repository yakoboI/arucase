 ...
 * Displays and manages Pre-Form One interview test results with automatic calculation
 */
...
const PreFormOneInterviewResults = () => {
  ...
  const downloadPDF = async (downloadFn, btnId, btnTextId, successMsg) => {
    ...
    try {
      ...
      if (btn && btnText) {
        btn.disabled = false;
        btnText.textContent = btnId.includes('Continuing') ? 'Download Continuing Results (PDF)' : 'Download Result (PDF)';
        btn.style.opacity = '1';
  ...
  const { data: subjects = [], isLoading: subjectsLoading } = useQuery({
    queryKey: ['preform-one-interview-subjects'],
    queryFn: async () => {
      try {
  ...
  // Fetch existing interview results
  const { data: existingResults = {}, isLoading: resultsLoading } = useQuery({
    queryKey: ['preform-one-interview-results', filter.year, filter.month],
    queryFn: async () => {
      try {
        const res = await preFormOneService.getInterviewResults(filter.year, filter.month);
  ...
  useEffect(() => {
    ...
    } else if (Object.keys(subjectScores).length > 0 && Object.keys(existingResults).length === 0) {
  ...
  }, [existingResults, subjectScores, students]);

  // Calculate results mutation
  const calculateResultsMutation = useMutation({
    mutationFn: async () => {
      return await preFormOneService.calculateInterviewResults(filter.year, filter.month);
    },
    onSuccess: (response) => {
      ...
      toast.success(response.data?.message || 'Interview results calculated and saved successfully!');
      queryClient.invalidateQueries(['preform-one-interview-results', filter.year, filter.month]);
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to calculate interview results');
    },
  });
  ...
  const saveResultMutation = useMutation({
    mutationFn: async (data) => {
      try {
        return await preFormOneService.saveInterviewResult(data);
      } catch (error) {
        throw error;
      }
    },
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to save interview result');
    },
  });
  ...
  const deleteResultMutation = useMutation({
    mutationFn: async (studentId) => {
      try {
        return await preFormOneService.deleteInterviewResult(studentId, filter.year);
      } catch (error) {
        throw error;
      }
    },
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to delete interview result');
    },
  });
  ...
  const handleDelete = (studentId) => {
    if (window.confirm('Are you sure you want to delete this interview result?')) {
  ...
  const handlePrint = () => downloadPDF(preFormOneService.downloadInterviewResultsPDF, 'downloadResultsBtn', 'downloadBtnText', 'PreFormOne_Interview_Results');
  ...
  const handleDownloadCSV = () => {
    ...
    // Add title row
    csv.push([`PRE-FORM ONE INTERVIEW RESULTS ${year}`]);
    csv.push([]);
    ...
    const link = document.createElement('a');
    const filename = `PreFormOne_Interview_Results_${year}`.replace(/\s+/g, '_') + '.csv';
    ...
    <div className="preform-one-results-page-container">
      <div className="excel-card preform-one-results">
        <div className="excel-card-header">
          <i className="fas fa-clipboard-check"></i> PRE-FORM ONE INTERVIEW RESULTS
        ...
        <>
          <div className="print-button-container">
            <button type="button" onClick={handlePrint} id="downloadResultsBtn" className="download-btn-monthly">
              <i className="fas fa-file-pdf"></i> <span id="downloadBtnText">Download Interview Results (PDF)</span>
            </button>
          ...
          <div className="report-header-section">
            ...
            <div className="test-info-bar">
              PRE-FORM ONE INTERVIEW RESULTS {filter.year} {filter.month !== 'all' ? `- ${filter.month}` : ''}
            </div>
          </div>

          {/* Student Selection Table - Like Form One Reports */}
                      ...
                      <tr key={student.id}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          ...
                          >
                            ...
                            {isGenerating[student.id] ? 'Generating...' : 'Generate Report'}
                          </button>
                        </td>
                      </tr>
                    );
          ...
          <div className="excel-card">
            <div className="excel-card-header">
              <i className="fas fa-table"></i> Interview Results
            </div>
            <div className="excel-card-body">
              <table className="excel-table">
                <thead>
                  <tr>
                    ...
                    <th className="sticky-col col-actions">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedStudents.map((student, index) => {
                    ...
                    return (
                      <tr key={student.id} className={gradeRowClass}>
                        ...
                        {subjects.map((subject) => {
                          const score = studentScores[subject.subject_code];
                          return (
                        ...
                        <td className="sticky-col col-actions">
                          <div className="action-buttons-row">
                            <button 
                              className="excel-btn primary small"
                              onClick={() => handleSave(student.admission_number)}
