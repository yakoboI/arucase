...
const scopeTrimArg = (process.argv[3] || 'preform-one-registration-route').replace(/^\./, '');
const SCOPE_TRIM = '.' + scopeTrimArg;
const SCOPE = SCOPE_TRIM + ' ';

if (!filePath || !fs.existsSync(filePath)) {
  console.error('Usage: node scope-preform-registration-shell.cjs <path-to-css> [scopeClassWithoutLeadingDot]');
  process.exit(1);
}
